...
def ScreenControl(offerL, offerR):
    ...
    """
    inputSignal = list(str(bin(offerL)[2:]))
    ...
    inputSignal = list(str(bin(offerR)[2:]))
...
##    return(trialList)
    ...
    for x in Trials:
        for y in range(Number)
            trialList.append([x[0], x[1], 1, 0])
            trialList.append([x[0], x[1], 0, 0])
            if laser:
                trialList.append([x[0], x[1], 1, 1])
                trialList.append([x[0], x[1], 0, 1])
    return(trialList)                           
